...
/// @notice Script to execute exactly 30 swaps (WETH->USDC) to generate fees
contract ThirtySwaps is Script {
    ...
    function run() external {
        ...
        console2.log("=== Executing 30 Swaps (WETH->USDC) ===");
        console2.log("Your address:", deployer);
        console2.log("Number of swaps: 30");
        ...
        uint256 totalWethNeeded = wethAmountPerSwap * 30;
        if (wethBalanceBefore < totalWethNeeded) {
            ...
            uint256 maxSwaps = wethBalanceBefore / wethAmountPerSwap;
            if (maxSwaps == 0) {
                console2.log("ERROR: Not enough WETH for even one swap!");
                return;
            }
            console2.log("Adjusted to", maxSwaps, "swaps");
        }
        ...
        for (uint256 i = 0; i < 30; i++) {
            ...
            console2.log("Amount (WETH):", wethAmountPerSwap / 1e18);
            ...
            try helper.swap(poolKey, swapParams, hookData) returns (BalanceDelta delta) {
                ...
                console2.log("Total Fees Accumulated - USDC:", totalFees0);
                console2.log("Total Fees Accumulated - WETH:", totalFees1);
            } catch Error(string memory errReason) {
                console2.log("Swap failed:", errReason);
                break;
            } catch {
        ...
        console2.log("\n=== Final Results ===");
        console2.log("Successful swaps:", successfulSwaps, "of 30");
